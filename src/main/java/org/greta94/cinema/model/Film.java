...
//        return (float) (nLike/this.votes.size() * 100);
    ...
    public Vote aVote(String username)
    {
        for (Vote vote: votes
             ) {
    ...
    /*film.aVote(username)!=null  and*/
    public boolean isLiked(String username)
    {
        Vote voted=aVote(username);
        return voted!=null && voted.isLike();


    }

    public void modifyVote(Vote newVote)
    {
        Vote oldVote = this.aVote(newVote.getUser().getUserName());
        if (oldVote.isLike()!=newVote.isLike())
        {
            oldVote.setLike(newVote.isLike());
        }
